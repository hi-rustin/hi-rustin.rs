...
大家好，我是 [Rustin](https://github.com/Rustin-Liu)。最近在做 6.824 的实验 lab2，今天想写一篇文章记录一下 Raft 领导人的实现。
...
下面我就简单描述一下 Raft 领导人选举机制我的实现和一些踩过的坑，我会尽可能的聚焦在实现实验的细节上，因为我可能对 Raft 目前的理解也比较浅显。**所以如果您对这篇文章感兴趣，请您先阅读论文和观看课程视频并尝试实现。**

## 领导人选举规则分析

在 Raft 中分别有三种角色分别是：Leader, Follower 和 Candidate。在整个系统运行正常运行过程中，会有一个 Leader 节点，其他的都是 Follower 节点。只有在需要重新选举的阶段，节点会尝试将自己转换为 
Candidate，然后向所有的节点发出投票 RPC 消息展开选举。

我觉得而一个比较好的切入思路就是搞明白我们需要何时进行 Leader 的选举？我们阅读论文会发现主要有两种情况需要进行选举，**一种就是当 Raft 集群启动之后开始第一次选举，另外一种就是 Leader 出现故障，无法使用 
HeartBeat 维持自己的统治，导致选举超时机制触发，节点开始尝试新的一轮的选举(需要注意的是，选举有可能在一个 Term 中没有结果，那么就在下个 Term 中继续选举直到选出 Leader)。**
...
**这些数据结构的大多数字段都是为了附加日志服务的，所以我们可以尽量的屏蔽掉目前无用的字段，目前对实验 A 有影响的就只有 Term 字段，所以我们在接下来的实现中确保 Term 处理逻辑正确即可。**

## 测试用例分析

在开始实现之前我们先来看看实验的测试用例。领导人实验实际上很简单，所有它的测试用力也很简单，主要有两个测试：
...
在 Make 函数中，我初始化了节点的基本信息，需要注意的是我为每个节点设置了一个叫做 **lastReceiveTime** 的字段，这个字段可以用作表示上次收到正常的 RPC 附加日志（在试验 A 中用作心跳）的时间，
当我发现我们接受到有效心跳的时间到现在超过了选举超时时间限制时，我们就将自己转换为候选人发起一轮新的选举。
	...
	for i := 0; i < len(rf.peers); i++ {
		if i != rf.me {
			go func(peerId int) {
                ...
                // 收到了有效投票，开始计算结果，如果获取系统中一般以上的选票，那就将该节点转换为领导人。并且开始放发出心跳维持自己的统治。
...
- 2.向每个节点发出投票请求（需要注意在试验中的 RPC 的消息和回复数据结构字段都要大写，否则会发送 RPC 失败）。
- 3.收到回复，如果投票人的 Term 比该节点大，那就将改节点转换为 Follower，否则就判断是否已经收到足够的多的选票，节点转换自己为领导人，并且启动一个 go 程开始发送心跳维持统治（go rf.replicaLogToPeer(j)）。
...
在上面的 RequestVote 函数中，我们主要就是根据分析中描述的规则进行投票。接下来我再看看当我们顺利成为领导人之后如何用心跳来维持自己的统治，需要正确的发送心跳，不停的刷新 Follower 的 lastReceiveTime：
...
上面的函数 replicaLogToPeer 和 sendAppendEntry 完成 RPC 不停发送和回复处理，我们再来看一看其他节点收到心跳之后该如何处理：
...
上面的 AppendEntries 函数主要的作用就是去刷新 Follower 的最近一次接受到有效心跳的时间，阻止该节点发生选举超时，从而维持自己的统治。到此为止，我们就就实现完了一个简单的领导人选举，思路其实还是很清晰，主要就是完全遵照论文的数据结构和规则来按部就班的实现。

在这里你可以找到实验 A 的[测试结果](https://github.com/Rustin-Liu/6.824/runs/542710174?check_suite_focus=true)。
